...
export const getNotificationStrategy = (notificationType) => {
  ...
  const persistentTypes = [NOTIFICATION_TYPES.CONTENT_STATUS, NOTIFICATION_TYPES.SYSTEM];
...
export const formatNotificationMessage = (type, context = {}) => {
  ...
  const baseMessages = {
    [NOTIFICATION_TYPES.LIKE]: `${actorName || 'Seseorang'} menyukai konten "${contentTitle}"`,
    [NOTIFICATION_TYPES.COMMENT]: `${actorName || 'Seseorang'} berkomentar pada "${contentTitle}"`,
    [NOTIFICATION_TYPES.SHARE]: `${actorName || 'Seseorang'} membagikan konten "${contentTitle}"`,
    [NOTIFICATION_TYPES.MENTION]: `${actorName || 'Seseorang'} menyebut Anda dalam "${contentTitle}"`,
  ...
  if (isDeleted && type === NOTIFICATION_TYPES.COMMENT) {
    message = `${actorName || 'Seseorang'} berkomentar pada "${contentTitle}" (komentar telah dihapus)`;
  }
...
export const formatNotificationForDisplay = (notification) => {
  ...
  return {
    ...
    invalidationInfo: notification.is_valid ? null : {
