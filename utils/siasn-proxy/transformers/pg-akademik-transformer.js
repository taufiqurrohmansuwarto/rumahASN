/**
 * Transform single peremajaan gelar akademik item from API to database format
 * @param {Object} item - Raw item from API
 * @returns {Object} Transformed item
 */
const transformPgAkademikItem = (item) => ({
  id: item?.id || null,
  nama: item?.nama || null,
  nip: item?.nip || null,
...
/**
 * Transform array of peremajaan gelar akademik items
 * @param {Array} items - Raw items from API
 * @returns {Array} Transformed items
 */
const transformPgAkademikData = (items) => {
  if (!Array.isArray(items)) {
    return [];
  }

  return items.map(transformPgAkademikItem).filter((item) => item.id !== null);
};

module.exports = {
  transformPgAkademikItem,
