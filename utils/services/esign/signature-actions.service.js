 ...
 */
export const signDocument = async (detailId, userId, signData = {}, mc, req = null) => {
  ...
  try {
    ...
    const base64File = await retrievePdfFromMinio(mc, requestData.document.file_path);
    console.log("✓ PDF retrieved, size:", base64File ? base64File.length : 0);

    // 2. Load signature logo
    console.log("5. Loading logo image...");
    ...
    console.log("7. Calling BSrE API to sign document...");
    ...
    console.log("8. Creating BSrE transaction record...");
    transactionId = nanoid();
    const now = new Date();

    // If signing failed, commit failed transaction and throw error
    if (bsreError) {
      console.log("✗ BSrE signing failed, creating failed transaction record...");
      ...
      try {
        await logBsreInteraction({
          ...
          http_method: "POST",
          http_status: 500,
          request_payload: { nik, signatureProperties, file_size: base64File.length },
          response_payload: {},
      ...
      try {
        ...
        console.error("✗ User activity logging error (non-critical):", logError.message);
      }
    ...
    // 6. Upload signed file back to Minio
    console.log("9. Uploading signed file back to Minio...");
    ...
    // 7. Update signature detail
    console.log("10. Updating signature detail...");
    ...
    // 8. Check if request is complete
    console.log("11. Checking if request is complete...");
    await checkAndCompleteRequest(detail.request_id);
    console.log("✓ Completion check done");

    // 9. Commit transaction
    console.log("12. Committing transaction...");
    await trx.commit();
    console.log("✓ Transaction committed");

    // 10. Log BSrE interaction (after commit)
    console.log("13. Logging BSrE interaction...");
    try {
      await logBsreInteraction({
        transaction_id: transactionId,
        action: "sign_document",
    ...
    // 11. Log user activity (after commit)
    console.log("14. Logging user activity...");
    try {
      await logSignatureAction({
        user_id: userId,
        action: "sign_document",
 ...
 * @returns {Promise<Object>} - Updated signature detail
 */
export const reviewDocument = async (detailId, userId, notes = "", req = null) => {
...
export const rejectDocument = async (detailId, userId, reason, req = null) => {
  ...
  await require("@/models/esign/esign-documents.model")
    .query()
    .patchAndFetchById(signatureRequest.document_id, {
      status: "rejected",
      updated_at: new Date(),
    });
