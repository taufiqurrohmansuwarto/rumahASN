...
const bentukPendampinganOptions = [
  { value: "kehadiran_persidangan", label: "Kehadiran di persidangan" },
  { value: "penyusunan_dokumen", label: "Penyusunan dokumen hukum" },
  { value: "konsultasi_tertulis", label: "Konsultasi tertulis" },
  { value: "lainnya", label: "Lainnya" },
];

const FormPendampinganHukum = ({ user, loading: userLoading, onSubmit, submitLoading }) => {
    ...
    <Stack gap="sm">
      ...
      <Paper p="md" radius="md" withBorder>
        <Form form={form} layout="vertical" requiredMark="optional" size="small">
          ...
          <Divider my="md" label={<Text size="xs" fw={600} c="blue">Bagian B. Data Perkara</Text>} labelPosition="left" />

          {/* Nomor Perkara */}
          <div>
            <Text size="xs" fw={500} mb={4}>Nomor Perkara (jika sudah ada):</Text>
            <Form.Item name="noPerkara" style={{ marginBottom: 0 }}>
              <Input size="small" placeholder="Contoh: 123/Pdt.G/2025/PN.Sby" prefix={<IconHash size={14} />} />
            </Form.Item>
          </div>
          ...
          {jenisPerkara?.includes("lainnya") && (
            <div style={{ marginTop: 8 }}>
              <Form.Item
                ...
                rules={[{ required: true, message: "Isi jenis perkara lainnya" }]}
                style={{ marginBottom: 0 }}
              >
                <Input size="small" placeholder="Tuliskan jenis perkara lainnya..." />
              </Form.Item>
            </div>
          )}
          ...
          <div style={{ marginTop: 16 }}>
            <Text size="xs" fw={500} mb={4}>Ringkasan Pokok Perkara:</Text>
            <Form.Item
              name="ringkasan_perkara"
              rules={[{ required: true, message: "Wajib diisi" }]}
              style={{ marginBottom: 0 }}
            >
              <Input.TextArea 
                size="small" 
                rows={5} 
                placeholder="Jelaskan ringkasan pokok perkara yang Anda hadapi..."
              />
            </Form.Item>
          </div>
          ...
          {/* Lampiran Dokumen */}
          <div style={{ marginTop: 16 }}>
            <Text size="xs" fw={500} mb={4}>Lampiran Dokumen (opsional):</Text>
              ...
              >
                <Group justify="center" gap={8}>
                  ...
                  <Stack gap={2} align="center">
                    ...
                    <Text size="xs" c="dimmed">Maks. 5 file, format PDF/DOC, ukuran ≤ 10MB</Text>
                  </Stack>
                </Group>
              </Upload.Dragger>
            </Form.Item>
          ...
          <Divider my="md" label={<Text size="xs" fw={600} c="blue">Bagian C. Permohonan Pendampingan</Text>} labelPosition="left" />

          {/* Bentuk Pendampingan */}
          <div>
            <Text size="xs" fw={500} mb={8}>Bentuk pendampingan yang diminta:</Text>
            <Form.Item
              name="bentukPendampingan"
              rules={[{ required: true, message: "Pilih minimal satu bentuk pendampingan" }]}
              style={{ marginBottom: 0 }}
            >
              <Checkbox.Group>
                <Stack gap={6}>
                  {bentukPendampinganOptions.map((opt) => (
                    <Checkbox key={opt.value} value={opt.value}>
                      <Text size="xs">{opt.label}</Text>
                    </Checkbox>
                  ))}
          ...
          {/* Input Lainnya Bentuk Pendampingan */}
          {bentukPendampingan?.includes("lainnya") && (
            <div style={{ marginTop: 8, marginLeft: 24 }}>
              <Form.Item
                name="bentukPendampinganLainnya"
                rules={[{ required: true, message: "Isi bentuk pendampingan lainnya" }]}
                style={{ marginBottom: 0 }}
              >
                <Input size="small" placeholder="Tuliskan bentuk pendampingan lainnya..." />
              </Form.Item>
            </div>
          )}

          <Divider my="md" label={<Text size="xs" fw={600} c="blue">Bagian D. Pernyataan</Text>} labelPosition="left" />
            ...
            >
              <Checkbox>
                <Text size="xs">
                  ...
                  ketentuan pendampingan hukum yang berlaku.
                </Text>
              </Checkbox>
            </Form.Item>
  ...
  const renderStep3 = () => (
    <Result
      ...
      subTitle="Tim hukum akan segera menghubungi Anda untuk proses pendampingan."
      extra={[
        <Button type="primary" key="list" onClick={() => router.push("/sapa-asn/pendampingan-hukum")}>
          Lihat Daftar
        </Button>,
  ...
  const getBentukLabel = (values) => {
    if (!values || values.length === 0) return "-";
    return values.map(v => {
      if (v === "lainnya") return formValues?.bentukPendampinganLainnya || "Lainnya";
      return bentukPendampinganOptions.find(k => k.value === v)?.label;
    }).join(", ");
  };
    ...
    <Center>
      <Box w={{ base: "100%", sm: 600, md: 700 }}>
        <Stack gap="md">
          ...
          {current < 2 && (
            <Paper p="sm" radius="md" withBorder>
              <Group justify="space-between">
                <Button danger size="small" onClick={() => router.push("/sapa-asn/pendampingan-hukum")}>
                  Batal
                </Button>
                <Space>
          ...
          >
            <Stack gap="sm">
              <Text size="sm">Ajukan permohonan pendampingan hukum dengan data berikut?</Text>
              <Descriptions column={1} size="small" bordered>
                <Descriptions.Item label="Jenis Perkara">
                  {getJenisPerkaraLabel(formValues?.jenisPerkara)}
                </Descriptions.Item>
                <Descriptions.Item label="Bentuk Pendampingan">
                  {getBentukLabel(formValues?.bentukPendampingan)}
                </Descriptions.Item>
                <Descriptions.Item label="Ringkasan">
                  {formValues?.ringkasan_perkara?.substring(0, 80)}...
                </Descriptions.Item>
              </Descriptions>
            </Stack>
          </Modal>
