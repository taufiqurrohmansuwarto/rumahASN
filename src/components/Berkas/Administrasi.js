...
const TombolDokumen = ({ tmt, file }) => {
  const { data } = useQuery(
    ...
    () => checkDocument({ tmt, file: file.code }),
  ...
  const downloadFile = async () => {
    try {
      ...
      const result = await downloadDocument({ tmt, file: file.code });
      if (result) {
      ...
      <Paper p="xs" radius="sm" withBorder>
        <Group justify="space-between" align="center">
          <Box>
            ...
            <Text size="xs" c="dimmed">Unduh SK, PERTEK, SPMT, dan PK berdasarkan TMT</Text>
          </Box>
          <Group spacing={4}>
