...
import { bupMasihAktif } from "@/services/dimensi-timeliness.services";
...
const BupMasihAktif = () => {
  ...
  const { data, isLoading, isFetching } = useQuery(
    ["bup-masih-aktif", router?.query],
    () => bupMasihAktif(router?.query),
    {
  ...
  const handleDownload = async () => {
    ...
    const result = await bupMasihAktif({ ...router?.query, limit: -1 });
