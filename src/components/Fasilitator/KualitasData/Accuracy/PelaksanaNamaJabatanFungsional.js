...
import { pelaksanaNamaJabatanFungsional } from "@/services/dimensi-accuracy.services";
...
const PelaksanaNamaJabatanFungsional = () => {
  ...
  const { data, isLoading, isFetching } = useQuery(
    ["pelaksana-nama-jabatan-fungsional", router?.query],
    () => pelaksanaNamaJabatanFungsional(router?.query),
    {
  ...
  const handleDownload = async () => {
    ...
    const result = await pelaksanaNamaJabatanFungsional({ limit: -1 });
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      sheet,
      "Pelaksana Nama Jabatan Fungsional"
    );
    ...
    saveAs(new Blob([excelBuffer]), "pelaksana-nama-jabatan-fungsional.xlsx");
...
export default PelaksanaNamaJabatanFungsional;
