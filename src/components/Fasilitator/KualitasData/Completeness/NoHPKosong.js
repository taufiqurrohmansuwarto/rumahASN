...
const NoHpKosong = () => {
  ...
  const { data, isLoading, isFetching } = useQuery(
    ["no-hp-kosong", router?.query],
    () => noHpKosong(router?.query),
    {
  ...
  const handleDownload = async () => {
    ...
    const result = await noHpKosong({ ...router?.query, limit: -1 });
    ...
    XLSX.utils.book_append_sheet(workbook, sheet, "No. HP Kosong");
    ...
    saveAs(new Blob([excelBuffer]), "no-hp-kosong.xlsx");
