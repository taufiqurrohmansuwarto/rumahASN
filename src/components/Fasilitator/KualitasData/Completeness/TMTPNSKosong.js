...
const TMTPNSKosong = () => {
  ...
  const handleDownload = async () => {
    ...
    const result = await tmtPnsKosong({ ...router?.query, limit: -1 });
    ...
    const sheetData = result?.data?.map((item) => ({
      ...
      OPD: item.opd_master,
      "TMT PNS": item.tmt_pns,
    }));

    const sheet = XLSX.utils.json_to_sheet(sheetData);
    XLSX.utils.book_append_sheet(workbook, sheet, "TMT PNS Kosong");
    ...
    saveAs(new Blob([excelBuffer]), "tmt-pns-kosong.xlsx");
