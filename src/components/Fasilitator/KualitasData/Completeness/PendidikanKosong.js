...
import { pendidikanKosong } from "@/services/dimensi-completeness.services";
...
const PendidikanKosong = () => {
  ...
  const { data, isLoading, isFetching } = useQuery(
    ["pendidikan-kosong", router?.query],
    () => pendidikanKosong(router?.query),
    {
  ...
  const handleDownload = async () => {
    ...
    const result = await pendidikanKosong({ ...router?.query, limit: -1 });
    ...
    XLSX.utils.book_append_sheet(workbook, sheet, "Pendidikan Kosong");
    ...
    saveAs(new Blob([excelBuffer]), "pendidikan-kosong.xlsx");
...
export default PendidikanKosong;
