...
const JabatanKosong = () => {
  ...
  const { data, isLoading, isFetching } = useQuery(
    ["jabatan-kosong", router?.query],
    () => jabatanKosong(router?.query),
    {
  ...
  const handleDownload = async () => {
    ...
    const result = await jabatanKosong({ ...router?.query, limit: -1 });
    ...
    XLSX.utils.book_append_sheet(workbook, sheet, "Jabatan Kosong");
    ...
    saveAs(new Blob([excelBuffer]), "jabatan-kosong.xlsx");
