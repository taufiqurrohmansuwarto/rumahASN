...
import { checkFotoPersonalByNipServices } from "@/services/master.services";
...
function CheckFotoPersonalByNip() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (refresh) =>
      checkFotoPersonalByNipServices({ nip, data: { refresh } }),
    onMutate: (refresh) => {
  ...
  const handleCheckFotoPersonal = () => {
    if (!nip) {
      message.error("NIP tidak ditemukan");
      return;
    }
...
export default CheckFotoPersonalByNip;
