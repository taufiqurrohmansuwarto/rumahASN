...
const BestieAIBanner = () => {
  const router = useRouter();
  const { data: session } = useSession();
  const screens = useBreakpoint();
  
  const isAsn = session?.user?.group === "MASTER" && session?.user?.role === "USER";
  const linkGambar = "https://siasn.bkd.jatimprov.go.id:9000/public/bestie-ai-title.png";
  
  const getText = () => {
    if (screens.xs && !screens.sm) {
      return "BestieAI";
    }
    return "BestieAI • Siap Melayani";
  };
    ...
    >
      ...
      >
        <Image
          ...
          fallback={<RobotOutlined style={{ fontSize: "16px", color: "#FF6600" }} />}
        />
      </div>
      <Text
        style={{
