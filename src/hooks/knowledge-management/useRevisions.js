...
export const useCreateRevision = () => {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const errorMessage = error.response?.data?.message || "Gagal membuat revision";
...
export const useUpdateRevision = () => {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const errorMessage = error.response?.data?.message || "Gagal mengupdate revision";
      message.error(errorMessage);
    },
  });
};
...
export const useSubmitRevision = () => {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const errorMessage = error.response?.data?.message || "Gagal submit revision";
      message.error(errorMessage);
    },
  });
};
...
export const useApproveRevision = () => {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const errorMessage = error.response?.data?.message || "Gagal memproses revision";
      message.error(errorMessage);
    },
  });
};
...
export const useRevisionManagement = (contentId, versionId = null) => {
  ...
  const handleUpdateRevision = useCallback(async (data, changeNotes = "") => {
  ...
  const handleSubmitRevision = useCallback(async (submitNotes = "") => {
    if (!contentId || !versionId) return null;
